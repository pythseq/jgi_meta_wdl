...
task singlem_remove_off_target {
  ...
  String stdout = "${in_name}.out.targeted.stdout"
  
   command {
     /singlem/bin/singlem summarise --singlem-packages `ls -d /pkgs/*spkg` --input-otu-table  ${input_csv} --output-otu-table ${otu_table_targeted} --exclude-off-target-hits 1> o 2> e; cat o e > ${stdout}
   }
    runtime {
        backend : "r5-120D-ceq"    
        docker : container
...
task singlem_cluster {
  ...
  String stdout = "${in_name}.out.targeted.clustered.stdout"  
   command {
     /singlem/bin/singlem summarise --cluster --cluster-id 0.89 --input-otu-table  ${input_target_csv} --output-otu-table ${otu_table_clustered} 1> o 2> e; cat o e > ${stdout}
   }
    runtime {
        backend : "r5-120D-ceq"    
        docker : container
...
task singlem_krona {
 ...
 String stdout = "${in_name}.out.targeted.clustered.krona.stdout" 
    command {
     /singlem/bin/singlem summarise --input_otu_tables ${input_clustered_csv} --krona ${plot_name} 1> o 2> e; cat o e > ${stdout}
    }
    runtime {
        backend : "r5-120D-ceq"    
        docker : container
